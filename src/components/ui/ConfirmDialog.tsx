  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 modal-overlay bg-black/30 dark:bg-black/50">
      <div className="glass-panel rounded-2xl shadow-2xl p-6 max-w-sm w-full animate-slide-up">
        ...
        <p className="text-sky-900 dark:text-sky-700 text-sm text-center mb-6 leading-relaxed">{message}</p>
        <div className="flex gap-3">
          <button
            onClick={onCancel}
            className="flex-1 px-4 py-2.5 rounded-xl border border-sky-300 bg-white/70 text-sky-700 hover:bg-white/90 transition-all font-semibold text-sm shadow-sm"
          >
            {cancelLabel}
          </button>
          <button
